...
% Of course, n is lower than nchan. n must be larger than 1 !!! 
...
%
% Input:
% bs: nchan x nchan x nchan in general complex tensor cross-spectral values for hnchan channels 
% n:  model order
% para: (optional) 
...
while kont==1;
    ...
    % log(alpha)/log(10)
     ...
     if k>5;
         diff_err=err_all(k-2)-err_all(k);
         if diff_err<10^(-6);kont=0;end
     end
     err_all(k+1)=err;
     bs_est=calc_bsmodel(a,d);
